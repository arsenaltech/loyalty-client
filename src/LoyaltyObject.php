...
use Arsenaltech\Loyalty\Util\Set;
 ...
 * @package Stripe
 */
class LoyaltyObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = Util::convertToStripeObject($v);
    ...
    public function refreshFrom($values, $partial = false)
    {
        ...
        if ($values instanceof LoyaltyObject) {
     ...
     */
    public function updateAttributes($values, $dirty = true)
    {
        foreach ($values as $k => $v) {
            ...
            if ($k === "metadata") {
                $this->_values[$k] = LoyaltyObject::constructFrom($v);
            } else {
                $this->_values[$k] = Util::convertToStripeObject($v);
            }
            if ($dirty) {
    ...
    public function serializeParameters($force = false)
    {
        ...
        foreach ($this->_values as $k => $v) {
            ...
            if ($force || $unsaved || $v instanceof LoyaltyObject) {
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if ($value === null) {
            ...
            if (Util::isList($value)) {
                ...
                return Util::convertToStripeObject($value)->serializeParameters();
            }
        } elseif ($value instanceof LoyaltyObject) {
